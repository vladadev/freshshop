...
if ($isOk) {
    $query = $conn->prepare(
        'INSERT INTO kontakt(ime_prezime, email, naslov, poruka) VALUES(:ime_prezime, :email, :naslov, :poruka)'
    );
    ...
    try {
        ...
        if (headers_sent()) {
            die(
                "Message is successfully sent, but redirect failed. Please click on this link: <a href='index.php?page=contact_us'>Here</a>"
            );
        } else {
        ...
        header(
            'location: index.php?page=contact_us&error=Not able to send message!'
        );
    }
}
