...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['submit-new-user']) {
    $url =
        'location: ' .
        NEW_BASE_URL .
        'admin-pannel/admin-index.php?admin-page=korisnici';
    $urlError =
        'location: ' .
        NEW_BASE_URL .
        'admin-pannel/admin-index.php?admin-page=korisnici&error=';
    $errorMsg = [];
    ...
    if (
        empty($imeKorisnika) &&
        empty($prezimeKorisnika) &&
        empty($emailKorisnika) &&
        empty($passwordKorisnika)
    ) {
    ...
    if (!preg_match($regPrezime, $prezimeKorisnika)) {
        array_push(
            $errorMsg,
            'Last name is not in a good format! Example: Peric'
        );
    }
    //PROVERA E-MAIL-A
    ...
    if (
        ...
        try {
            ...
            $urlUpdate =
                'location: ' .
                NEW_BASE_URL .
                'admin-pannel/admin-index.php?admin-page=user-update';
            header(
                $urlUpdate .
                    '&error=Neuspesan update&artikal_id=' .
                    $params['id']
            );
        }
    ...
    header(
        'location: ' .
            NEW_BASE_URL .
            'admin-pannel/admin-index.php?admin-page=korisnici&errorInsert=Neovlasceni pristup'
    );
}
