...
if (in_array($imageFileType, $extensions_arr)) {
    // Insert record
    $queryImageInsert = $conn->prepare(
        'INSERT INTO slika(putanja, alt) VALUES (:putanja, :alt)'
    );
    ...
    $querySlikaId = $conn->prepare(
        'SELECT id_slika FROM slika ORDER BY id_slika DESC LIMIT 1'
    );
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['submit-new-article']) {
    $url =
        'location: ' .
        NEW_BASE_URL .
        'admin-pannel/admin-index.php?admin-page=artikli';
    ...
    if (
        ...
        try {
            ...
            $queryArtikalId = $conn->prepare(
                'SELECT id_artikal FROM artikal ORDER BY id_artikal DESC LIMIT 1'
            );
            $queryArtikalId->execute();
            $resultArtikalId = $queryArtikalId->fetchAll(PDO::FETCH_ASSOC);
            $resultToObj = (object) $resultArtikalId[0];
            ...
            //Povezati Sliku i Artikal u bazi.

            //Napraviti insert nad tabelom slika_artikal
            //artikal_id = $id_artikla - slika_id = $id_slike

            $queryImageInsert = $conn->prepare(
                'INSERT INTO slika_artikal(artikal_id, slika_id) VALUES (:lastArtikalId, :lastSlikaId)'
            );
            $queryImageInsert->bindParam(
                ':lastArtikalId',
                $idPoslednjegArtikla
            );
    ...
    header(
        'location: ' .
            NEW_BASE_URL .
            'admin-pannel/admin-index.php?admin-page=artikli&errorInsert=Neovlasceni pristup'
    );
}
